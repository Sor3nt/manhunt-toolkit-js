...
function computeAutoBoneMap( skeleton ){
    // const auxBoneMap = Object.keys(AnimationRetargeting.boneMap);
    const auxBoneMap = skeleton.bones.map((b) => b.name );
    let bones = skeleton.bones;
    let result = {
    ...
    for(let i = 0; i < auxBoneMap.length; i++) {
        ...
        for( let j = 0; j < bones.length; ++j ){
            ...
            if(name.toLowerCase().includes(auxName.toLowerCase()) || name.toLowerCase().includes(AnimationRetargeting.boneMap[auxName]?.toLowerCase())) {
